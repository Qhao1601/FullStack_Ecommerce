...
const useApi = () => {
    ...
    const usePagiante = <T>(endpoint: string, queryParams?: Record<string, string>, options?: { enable?: boolean }) => {
    ...
    // này xuwrlys khuyến mãi
    const usePagiantee = <T>(endpoint: string, queryParams?: Record<string, string>, options?: { enable?: boolean }) => {
        const fullUrl = buildUrlWithQueryString(endpoint, queryParams)

        return useQuery<IApiResponse<IPaginate<T> | T[], unknown>, Error>({
            queryKey: [endpoint, queryParams],
            queryFn: () => baseService.paginate<T>(fullUrl),
            enabled: options?.enable ?? true
        })
