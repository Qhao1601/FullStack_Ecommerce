...
const config: IConfigModule<IAttributes> = {
    ...
    columns: [
        ...
        { key: 'attributeCatalogue', label: 'Tên nhóm thuộc tính', className: 'text-left w-[25%]' },
...
const TableRowComponent = React.memo(({
    ...
    <TableRow key={item.id} className="hover:bg-gray-50">
        ...
        <TableCell className="text-left">{item.attributeCatalogue.name}</TableCell>
        ...
        <TableCell>
            <div className="flex items-center justify-center gap-2">
                <Link to={`/attributes/edit/${item.id}`}>
            ...
            <div className="flex flex-1 flex-col gap-4 p-4 bg-[#f3f3f4] pt-[20px]">
                <Card className="rounded-[5px]">
                    ...
                    <CardFooter className="borderr-t">
                        {!isLoading && attributes && 'data' in attributes && isApiSuccessResponse(attributes) && 'links' in attributes.data && (
                            <PaginateComponent
                                links={attributes.data.links}
                                // click chuyển trang
                                onPageChange={handlePageChange}
                                from={attributes.data.from}
...
export default memo(AttributesIndex)