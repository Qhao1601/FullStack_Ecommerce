...
const config: IConfigModule<IAttributeCatalogues> = {
    endpoint: 'v1/attribute_catalogues',
    title: 'Quản lý thuộc tính',
    description: 'Quản lý thông tin danh sách thuộc tính',
    createLink: '/attribute_catalogues/create',
    extraFilters: [...commonExtraFilter],
    columns: [
        { key: 'checkbox', label: '', className: 'w-[50px] text-center' },
        { key: 'id', label: 'ID', className: 'text-left w-[80px]' },
        { key: 'name', label: 'Tên thuộc tính', className: 'text-left w-[25%]' },
    ...
    <TableRow key={item.id} className="hover:bg-gray-50">
        ...
        <TableCell>
            <div className="flex items-center justify-center gap-2">
                <Link to={`/attribute_catalogues/edit/${item.id}`}>
                ...
                <Card className="rounded-[5px]">
                    ...
                    <CardContent>
                        ...
                        <CustomTable
                            columns={columns}
                            data={tableItems}
                            render={(item: IAttributeCatalogues) => (
                                <TableRowComponent
                                    key={item.id}
                                    item={item}
                    ...
                    <CardFooter className="borderr-t">
                        {!isLoading && attributeCatalogues && 'data' in attributeCatalogues && isApiSuccessResponse(attributeCatalogues) && 'links' in attributeCatalogues.data && (
                            <PaginateComponent
                                links={attributeCatalogues.data.links}
                                // click chuyển trang
                                onPageChange={handlePageChange}
                                from={attributeCatalogues.data.from}
                                to={attributeCatalogues.data.to}
                                total={attributeCatalogues.data.total}
                                currentPage={attributeCatalogues.data.current_page}
                            />
                        )}
                    </CardFooter>
                </Card>
...
export default memo(AttributeCatalogueIndex)