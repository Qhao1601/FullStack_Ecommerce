...
const config: IConfigModule<IProductCatalogue> = {
    endpoint: 'v1/product_catalogues',
    title: 'QL nhóm bài viết',
    description: 'Quản lý thông tin danh sách nhóm bài viết',
    createLink: '/product_catalogues/create',
    ...
    columns: [
        ...
        // { key: 'users_count', label: 'Số bài viết', className: 'text-center w-[120px]' },
...
const TableRowComponent = React.memo(({
    ...
    <TableRow key={item.id} className="hover:bg-gray-50">
        ...
        <TableCell className="text-left">{"|---".repeat(item.level > 0 ? item.level - 1 : 0)}{item.name}</TableCell>
        ...
        <TableCell>
            <div className="flex items-center justify-center gap-2">
                <Link to={`/product_catalogues/edit/${item.id}`}>
            ...
            <div className="flex flex-1 flex-col gap-4 p-4 bg-[#f3f3f4] pt-[20px]">
                <Card className="rounded-[5px]">
                    ...
                    <CardFooter className="borderr-t">
                        {!isLoading && productCatalouges && 'data' in productCatalouges && isApiSuccessResponse(productCatalouges) && 'links' in productCatalouges.data && (
                            <PaginateComponent
                                links={productCatalouges.data.links}
                                // click chuyển trang
                                onPageChange={handlePageChange}
                                from={productCatalouges.data.from}
                                to={productCatalouges.data.to}
                                total={productCatalouges.data.total}
                                currentPage={productCatalouges.data.current_page}
                            />
                        )}
                    </CardFooter>
                </Card>
...
export default memo(ProductCatalogueIndex)