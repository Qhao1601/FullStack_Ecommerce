...
import { IProductCatalogue, IProductCatalogueRequest } from "@/interfaces/products/product-catalogues.interface";
...
const formConfig = (isUpdateMode: boolean) => ({
    ...
    routerIndex: '/product_catalogues',
    mapToRequest: (values: {
        name: string,
        parentId: string,
        publish: string,
    ...
    }, userId?: number): IProductCatalogueRequest => {
        ...
        return formData as unknown as IProductCatalogueRequest
...
const ProductCataloguesSave = () => {
    const api = useApi()
    const { id } = useParams()
    const isUpdateMode = !!id
    const parentCategory = api.usePagiante('/v1/product_catalogues', { sort: 'lft, asc', type: 'all' })
    const { onSubmit, form, record } = useCurd<IProductCatalogue, IProductCatalogueRequest, TFromValues>(formConfig(isUpdateMode))
    ...
    const ParentCategoriOptions = useMemo(() => {
        if (parentCategory && isApiSuccessResponse(parentCategory.data)) {
            const CategoryData = parentCategory.data.data as IProductCatalogue[]
            return [
...
export default ProductCataloguesSave