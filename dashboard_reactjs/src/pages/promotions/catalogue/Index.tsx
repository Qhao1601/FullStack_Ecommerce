...
const config: IConfigModule<IPromotionCatalogues> = {
    endpoint: 'v1/promotion_catalogues',
    title: 'Quản lý nhóm khuyến mãi',
    description: 'Quản lý thông tin danh sách nhóm khuyến mãi',
    createLink: '/promotion_catalogues/create',
    extraFilters: [...commonExtraFilter],
    columns: [
        { key: 'checkbox', label: '', className: 'w-[50px] text-center' },
        { key: 'id', label: 'ID', className: 'text-left w-[80px]' },
        { key: 'name', label: 'Tên nhóm khuyến mãi', className: 'text-left w-[25%]' },
        { key: 'user_id', label: 'Người tạo', className: 'text-left w-[100px]' },
...
const TableRowComponent = React.memo(({
    ...
    <TableRow key={item.id} className="hover:bg-gray-50">
        ...
        <TableCell className="text-left">{item.user_id.name}</TableCell>
        <TableCell className="text-center">
            <Switch
        ...
        <TableCell>
            <div className="flex items-center justify-center gap-2">
                <Link to={`/promotion_catalogues/edit/${item.id}`}>
                ...
                <Card className="rounded-[5px]">
                    ...
                    <CardContent>
                        ...
                        <CustomTable
                            columns={columns}
                            data={tableItems}
                            render={(item: IPromotionCatalogues) => (
                                <TableRowComponent
                                    key={item.id}
                                    item={item}
                    ...
                    <CardFooter className="borderr-t">
                        {!isLoading && promotionCatalogues && 'data' in promotionCatalogues && isApiSuccessResponse(promotionCatalogues) && 'links' in promotionCatalogues.data && (
                            <PaginateComponent
                                links={promotionCatalogues.data.links}
                                // click chuyển trang
                                onPageChange={handlePageChange}
                                from={promotionCatalogues.data.from}
                                to={promotionCatalogues.data.to}
                                total={promotionCatalogues.data.total}
                                currentPage={promotionCatalogues.data.current_page}
                            />
                        )}
                    </CardFooter>
                </Card>
...
export default memo(PromotionCatalogueIndex)