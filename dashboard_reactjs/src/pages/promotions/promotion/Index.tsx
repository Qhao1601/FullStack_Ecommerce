...
import { IPromotions } from "@/interfaces/promotions/promotion.interface"
...
const config: IConfigModule<IPromotions> = {
    endpoint: 'v1/promotions',
    title: 'Quản lý khuyến mãi',
    description: 'Quản lý thông tin danh sách khuyến mãi',
    createLink: '/promotions/create',
    extraFilters: [...commonExtraFilter],
    columns: [
        { key: 'checkbox', label: '', className: 'w-[50px] text-center' },
        { key: 'id', label: 'ID', className: 'text-left w-[80px]' },
        { key: 'name', label: 'Tên khuyến mãi', className: 'text-left w-[25%]' },
        { key: 'promotionCatalogues', label: 'Nhóm khuyến mãi', className: 'text-left w-[25%]' },
        { key: 'creator', label: 'Người tạo', className: 'text-left w-[11%]' },
        { key: 'startDate', label: 'Ngày tạo', className: 'text-left w-[7%]' },
        { key: 'endDate', label: 'Ngày kết thúc', className: 'text-left w-[7%]' },
        { key: 'publish', label: 'Trạng thái', className: 'text-center w-[80px]' },
...
const TableRowComponent = React.memo(({
    ...
    <TableRow key={item.id} className="hover:bg-gray-50">
        ...
        <TableCell className="text-left">{item.promotionCatalogues && item.promotionCatalogues.name ? item.promotionCatalogues.name : "Không có nhóm"}</TableCell>
        <TableCell className="text-left">{item.users}</TableCell>
        <TableCell className="text-left text-blue-500 ">{item.startDate}</TableCell>
        <TableCell className="text-left text-red-600 ">{item.endDate}</TableCell>
        <TableCell className="text-center">
            <Switch
        ...
        <TableCell>
            <div className="flex items-center justify-center gap-2">
                <Link to={`/promotions/edit/${item.id}`}>
            ...
            <div className="flex flex-1 flex-col gap-4 p-4 bg-[#f3f3f4] pt-[20px]">
                <Card className="rounded-[5px]">
                    ...
                    <CardFooter className="borderr-t">
                        {!isLoading && promotions && 'data' in promotions && isApiSuccessResponse(promotions) && 'links' in promotions.data && (
                            <PaginateComponent
                                links={promotions.data.links}
                                // click chuyển trang
                                onPageChange={handlePageChange}
                                from={promotions.data.from}
                                to={promotions.data.to}
                                total={promotions.data.total}
                                currentPage={promotions.data.current_page}
                            />
                        )}
                    </CardFooter>
                </Card>
...
export default memo(PromotionsIndex)