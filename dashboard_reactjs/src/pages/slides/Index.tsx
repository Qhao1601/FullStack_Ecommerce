...
import { ISlides } from "@/interfaces/slides/slides.interface"
...
const config: IConfigModule<ISlides> = {
    endpoint: 'v1/slides',
    title: 'QL slides',
    description: 'Quản lý thông tin danh sách slides',
    createLink: '/slides/create',
    extraFilters: [...commonExtraFilter],
    columns: [
        { key: 'checkbox', label: '', className: 'w-[50px] text-center' },
        { key: 'id', label: 'ID', className: 'text-left w-[80px]' },
        { key: 'name', label: 'Tên slides', className: 'text-left w-[25%]' },
        { key: 'keyword', label: 'Từ khóa', className: 'text-left w-[120px]' },
        { key: 'description', label: 'Mô tả slides', className: 'text-center w-[120px]' },
...
const TableRowComponent = React.memo(({
    ...
    <TableRow key={item.id} className="hover:bg-gray-50">
        ...
        <TableCell className="text-left">{item.keyword}</TableCell>
            ...
            <div className="flex flex-1 flex-col gap-4 p-4 bg-[#f3f3f4] pt-[20px]">
                <Card className="rounded-[5px]">
                    ...
                    <CardFooter className="borderr-t">
                        {!isLoading && slides && 'data' in slides && isApiSuccessResponse(slides) && 'links' in slides.data && (
                            <PaginateComponent
                                links={slides.data.links}
                                // click chuyển trang
                                onPageChange={handlePageChange}
                                from={slides.data.from}
                                to={slides.data.to}
                                total={slides.data.total}
                                currentPage={slides.data.current_page}
                            />
                        )}
                    </CardFooter>
                </Card>
...
export default memo(SlidesIndex)