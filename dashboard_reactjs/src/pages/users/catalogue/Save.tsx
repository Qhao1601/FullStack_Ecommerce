...
const UserCatalogueSave = () => {
    ...
    const { onSubmit, form, record } = useCurd<IUserCatalogue, IUserCatalogueRequest, TFromValues>(formConfig)
    const api = useApi()
    const { data: permissions } = api.usePagiante<IPermission[]>('v1/permissions', { type: 'all', sort: 'name,asc' })
